//TCP Client code created with help from my professor, Nate Evans
...
bool TCPClient::get_send_data(char *buf, unsigned int buf_len) {
  ...
  if (buf_len < send_buf_offset) {
    std::cerr << "Need to send " << send_buf_offset << " bytes but buffer is only " << buf_len << " bytes." << std::endl;
  }
...
struct sockaddr_storage *TCPClient::get_client_address() {
  static struct sockaddr_storage null_address;
  if (0 == memcmp(&client_addr, &null_address, sizeof(struct sockaddr_storage))) {
    return NULL;
  } else {
    return &client_addr;
  }
