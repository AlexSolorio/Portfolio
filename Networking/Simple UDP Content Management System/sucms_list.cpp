...
/**
 * UDP client for sending/receiving "list" commands from server
 *
...
int main(int argc, char *argv[]) {
  ...
  connectHeader.sucms_msg_length = htons(sizeof(CommandMessage)+strlen(username.c_str()));
  
  //Create CommandMessage
  CommandMessage cmdMsg;
  cmdMsg.command = htons(COMMAND_LIST);
  ...
  ret = sendto(udp_socket, &sendBuffer, 2048, 0,
  ...
  getResult.command_type = htons(COMMAND_LIST);
  getResult.result_id = htons(sResponse.result_id);
  //message number not needed for LIST command
  ...
  int sendSize = sizeof(clientHeader) + sizeof(cmdMsg2) + username.length() + sizeof(SUCMSClientGetResult);
  
  // Send the data to the destination.
  ret = sendto(udp_socket, &sendBuffer2, sendSize, 0,
               dest_addr, client_address_length);
  ...
  //////////////// Receive from server /////////////////
  //////////////////////////////////////////////////////
  std::cout<<"Message count: "<<sResponse.message_count<<std::endl;
  for (int i = 0; i < sResponse.message_count; i++){
    ret = recvfrom(udp_socket, recv_buf, 2047, 0, dest_addr, &client_address_length);
    ...
    //Parse List Result
    SUCMSFileListResult listResult;
    memcpy(&listResult, &recv_buf[sizeof(SUCMSHeader)], sizeof(SUCMSFileListResult));
    //Convert to Host Byte Order
    listResult.message_number = ntohs(listResult.message_number);
    listResult.result_id = ntohs(listResult.result_id);

    //Loop through all filenames/sizes in message
    int offset = sizeof(SUCMSHeader) + sizeof(SUCMSFileListResult);
    while (true){
      if (offset >= ret){
        break;
      }
      ...
      memcpy(&fileInfo, &recv_buf[offset], sizeof(SUCMSFileInfo));
      //Convert to Host Byte Order
      fileInfo.filename_len = ntohs(fileInfo.filename_len);
      fileInfo.filesize_bytes = ntohl(fileInfo.filesize_bytes);
      
      offset += sizeof(SUCMSFileInfo);
      
      //Print information
      std::cout << "File list entry: ";
      for (int j = 0; j < fileInfo.filename_len; j++)
        std::cout << recv_buf[offset + j];
      std::cout << " of size " << fileInfo.filesize_bytes << " bytes.\n";
      
      offset += fileInfo.filename_len;
    }
  }
